...
class CustomDDPMScheduler(SchedulerMixin, ConfigMixin):
    """
    ...
    def step(
        ...
        #! Multiply by the conditioning matrix G!
        max_len = pred_prev_sample.shape[1]
        G = (
            1
            / (math.sqrt(2 * max_len))
            * torch.eye(max_len, device=pred_prev_sample.device)
        )
        # Double the variance for the first component
        G[0, 0] *= math.sqrt(2)

        pred_prev_sample = pred_prev_sample + torch.matmul(G, variance)
